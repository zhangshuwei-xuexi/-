 ...
 */
 ...
 function deleteOverdue(){
     ...
     var url='/deleteOverdueQuestionnaires';
     commonAjaxPost(true,url,data,deleteOverdueSuccess);
 }
 function deleteOverdueSuccess(result){
     if (result.code == "666") {
         layer.msg(result.message,{icon: 1});
         history.go(0);
     ...
     if (result.code == "666") {
         ...
         if (data.length) {
             for (var i = 0; i < data.length; i++) {
                 ...
                 // setCookie("projectContent",projectInfo.project_content);
                 if (projectName.length >= 25) {
                     projectName = projectName.substring(0, 26) + "...";
                 }
                 setCookie("projectId",projectInfo.id);
                 ...
                 text += "             <a class=\"pull-left item-remind\" id=\"projcetUpdate" + i + "\" href=\"javascript:void(0)\" onclick=\"editProject(" + "'" + projectInfo.id + "'" + "," + "'" + projectInfo.project_name + "'" + "," + "'" + projectInfo.project_content + "'" + ")\"><i class=\"icon remind-icon\"></i>编辑</a>";
                 ...
                 text += " <table style='border-collapse: collapse' border='2'>";
                 text += "<thead>";
                 text +="<tr>"
                 text +="<th style='text-align: center' width='280px'>";
                 text +="<div style='box-sizing: border-box' >序号</div>";
                 text +="<div class=fht-cell></div>"
                 text +="</th>"
                 text +="<th style='text-align: center' width='280px' >";
                 text +="<div style='box-sizing: border-box' >问卷名称</div>";
                 text +="<div class=fht-cell></div>"
                 text +="</th>"
                 text +="<th style='text-align: center' width='280px'>";
                 text +="<div style='box-sizing: border-box' >开始时间</div>";
                 text +="<div class=fht-cell></div>"
                 text +="</th>"
                 text +="<th style='text-align: center' width='280px'>";
                 text +="<div style='box-sizing: border-box'>结束时间</div>";
                 text +="<div class=fht-cell></div>"
                 text +="</th>"
                 text +=  "</thead>"
                 text +="<tbody id='body'>"
 ...
 function getQuestionnaire1(){
     ...
     commonAjaxPost(false,url,data,getQuestionnaireSuccess1);
 }
 function getQuestionnaireSuccess1(result){
     ...
     if (result.code == "666") {
        ...
        for(var i=0;i<questionnaires.length;i++){
            ...
            text += "    <td style=\"text-align: center;color: #d9534f\" colspan=\"1\">"+current.start_time+"</td>";
            text += "    <td style=\"text-align: center;color: #d9534f\" colspan=\"1\">"+current.end_time+"</td>";
            text += "</tr>";
            $("#body").append(text);
        }
