    ...
    if (res.code == "666") {
       ...
       if (res.data.length == 0) {
        ...
        for (var i = 0; i < res.data.length; i++) {
            ...
            '                            <span class="pull-left">'+res.data[i].question_name+'</span>' +
            '                        </div>' +
            '                        <div class="details-info">丰富题型，强大逻辑</div>' +
            '                        <div class="details-info">问卷密码，红包抽奖</div>' +
            '                    </div>' +
            '                    <div class="clear dotted-line--solid"></div>\n' +// '"' + ',' + '"' + res.data[i].questionName + '"'
            '                    <a href="#" class="btn btn-blue-frame main__btn--new" onclick=\'importModal('+ '"' + res.data[i].id + '"' + ',' + '"' + res.data[i].question_name + '"' + ',' + '"' + res.data[i].question_content + '"' + ',' + '"' + getCookie('projectId') + '"' +')\'>导入</a>' +
            '                </div>';
...
function queryQuestionnaireMouldSuccess(res) {
    ...
    if (res.code == "666") {
        ...
        for (var i = 0; i < res.data.length; i++) {
            ...
            //     '                    <a href="javascript:void(0)" class="btn btn-blue-frame main__btn--new" style="left:71%" onclick=\'importModal(' + '"' + getCookie("projectId") + '"' + ')\'>导入</a>' +
            //     '                </div>';
            // $("#typeQuestion").append(questionnaireModal_div);
        }
    } else if (res.code == "333") {
...
//导入模板
function importModal(questionId1,questionName1,questionContent1,projectId) {//可以改变参数获得模板问卷的名字
    //吧questionTitle和question数据正过来
    var data={
        "id":questionId1,
    };
    var url="/queryQuestionnaireById";
    commonAjaxPost(false,url,data,function (res) {
        questionTitle=res.data.question_title;
        question1=res.data.question;
    });

    var questionId=questionId1;
    var questionName=questionName1;
    ...
    setCookie('questionList',question1);
