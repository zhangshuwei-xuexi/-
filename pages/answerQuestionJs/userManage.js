...
//
function descArrange1(){
    $("#userTable").bootstrapTable('destroy');
    var oTableInit=new descArrange();
    oTableInit.Init();
}
function descArrange(){

    var oTableInit=new Object();
    oTableInit.Init = function () {
        alert("jinlaile");
        $('#userTable').bootstrapTable({
            url: httpRequestUrl + '/admin/queryUserListDESC',         //请求后台的URL（*）
            method: 'POST',                      //请求方式（*）
            ...
            sortable:true,
            sortOrder: "asc",                   //排序方式
            queryParamsType: '',//加了之后将数据全部显示了出来
            dataType: 'json',
            ...
            responseHandler: function (res) {
                //console.log(res);
                if(res.code == "666"){//应该在这里的得到数据库里的数据，然后进行展示
...
function TableInit() {
    ...
    // 得到查询的参数,这里是提交到服务器端的参数,笨方法指定可以了
    function queryParams(params) {
        var userName = $("#keyWord").val();
        //console.log(userName);
        var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
