...
function setQuestion(result) {
    ...
    // var arr = result.data.question_name.split("");
    // for(var i=0;i<arr.length;i++){
    //     if(arr[i]==="（"){
    //         var questionName1 = result.data.question_name.split("（")[0];
    //         var questionName2 = "（" + result.data.question_name.split("（")[1];
    ...
    // }
    flag=false;
    $('#questionnaireTittle1').html(result.data.question_name);;
    $('.officialTips').html(result.data.question_content);
    questionList = result.data.question;
    var questionList1=JSON.parse(questionList);
    // alert(JSON.stringify(questionList));
    if(questionList!=null) {
        for (var i = 0; i < questionList1.length; i++) {
            var questionType = questionList1[i].questionType;
            switch (questionType) {
                case "0": //单选
                case "1": //多选
                    ...
                    for (var j = 0; j < questionList1[i].questionOption.length; j++) {
                        //题目选项
                        var option_word = questionList1[i].questionOption[j].optionWord;
                        //判断是多选还是单选
                        var inputType = 'radio';
                        if (questionType == "1") {
                            inputType = 'checkbox';
                    ...
                    if(questionList[i].important == "必答题") {
                        question_div = $(question_div).append('<div><label><span class="req">*</span><span class="title">' + (i + 1) + '. ' + questionList1[i].questionTitle + '</span></label></div>');        //添加标题
                    }else {
                        question_div = $(question_div).append('<div><label><span>&nbsp;&nbsp;&nbsp;</span><span class="title">' + (i + 1) + '. ' + questionList1[i].questionTitle + '</span></label></div>');        //添加标题
                    }
