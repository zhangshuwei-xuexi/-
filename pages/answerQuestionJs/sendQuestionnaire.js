...
var sendTime = "";
var i=decodeURI(location.search).split("?")[1].split("=")[1];
var questionId = getCookie("currentQuestionnaireId"+i);
...
commonAjaxPost(false, url, da, function (res) {
        dataId=res.data.data_id;
});
//按钮事件
...
$('#image').change(function (e) {
    ...
    fileReader.onload = function (ev) {
        ...
        if (persons.length != 0) {
            if (!persons[0].no || !persons[0].answerNum || !persons[0].answerName || !persons[0].answerBelong  || !persons[0].answerEmail) {
                layer.msg('数据模板不正确');
